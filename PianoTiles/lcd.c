...
#define DATA_BITS ((1 << PD5)|(1 << PD4)|(1 << PD3)|(1 << PD2))
#define CTRL_BITS ((1 << PB4)|(1 << PB3)) 
// enable is 3
...
void lcd_init(void)
{
    ...
    DDRB |= ((1 << PB4)|(1 << PB3));// Take care not to affect any unnecessary bits
...
void lcd_writenibble(unsigned char lcdbits)
{
    ...
    PORTD &= ~((1 << PD5)|(1 << PD4)|(1 << PD3)|(1 << PD2)); // making PD2,4,6,7 0
    PORTD |= (lcdbits & ((1 << PD5)|(1 << PD4)|(1 << PD3)|(1 << PD2))); // set bit PD2,4,6,7 

    /* Make E signal (PB2) go to 1 and back to 0 */
    PORTB |= (1<< PB3); // make enable signal go to 1
    PORTB |= (1<< PB3); // double the command to delay the enable signal
    PORTB &= ~(1 <<PB3); // back to 0
}
